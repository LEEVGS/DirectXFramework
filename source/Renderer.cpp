...
namespace dae {
	...
	void Renderer::CreateMesh()
	{
		...
		//Load main mesh
		Utils::ParseOBJ("Resources/winstonnew.obj", vertices, indices);

		paths.effect = L"Resources/PosCol3DBasic.fx";
		paths.diffuse = "Resources/wintonDiffuse.png";

		m_pMeshes.push_back(new Mesh{ m_pDevice, vertices, indices, paths });
		m_pMeshes[m_pMeshes.size() - 1]->SetScale({ 10.f,10.f,10.f });
		m_pMeshes[m_pMeshes.size() - 1]->SetPosition({ 0.f,-15.f,0.f });
		m_pMeshes[m_pMeshes.size() - 1]->RotateY(-90.f);
		vertices.clear();
		indices.clear();
		paths.Clear();
	}
